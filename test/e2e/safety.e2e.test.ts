...
describe('e2e: safety rules and writes', () => {
  ...
  it('blocks a blacklisted operation separated by a comment', async () => {
    // MySQL accepts a comment wherever whitespace is legal, so the guard has to
    // see through it — otherwise this really does drop the database.
    const {code, stderr} = await runCli(['mysql', 'query', 'DROP/* sneaky */DATABASE mq_e2e_alt'], configDir)
