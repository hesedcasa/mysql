...
describe('e2e: query execution', () => {
  ...
  it('returns rows for a SELECT as JSON', async () => {
    const payload = await runCliJson<{data: {result: Row[]}; success: boolean}>(
      ['mysql', 'query', "SELECT id, email, name FROM users WHERE status = 'active' ORDER BY id"],
      configDir,
    )
  ...
  it('returns an empty result set without failing', async () => {
    const payload = await runCliJson<{data: {result: Row[]}; success: boolean}>(
      ['mysql', 'query', "SELECT id FROM users WHERE email = 'nobody@example.com'"],
      configDir,
    )
  ...
  it('applies the default LIMIT even when a column name contains "limit"', async () => {
    const payload = await runCliJson<{data: {result: Row[]}}>(
      ['mysql', 'query', 'SELECT id AS limit_reached FROM metrics'],
      configDir,
    )
  ...
  it('reports analysis warnings and the applied limit on stderr', async () => {
    const {stderr, stdout} = await runCliOk(['mysql', 'query', 'SELECT * FROM metrics'], configDir)
  ...
  it('formats results as toon with --toon', async () => {
    const {stdout} = await runCliOk(
      ['mysql', 'query', 'SELECT id, name FROM users ORDER BY id LIMIT 2', '--toon'],
      configDir,
    )
  ...
  it('serialises dates, NULLs and binary payloads in toon output', async () => {
    const {stdout} = await runCliOk(
      ['mysql', 'query', 'SELECT id, note, payload, recorded FROM quirky ORDER BY id', '--toon'],
      configDir,
    )

    expect(stdout).to.include('2024-03-01T')
    // 0x00FF10 base64-encoded.
    expect(stdout).to.include('AP8Q')
  })

  it('renders a box table for human output', async () => {
    const {stdout} = await runCliOk(['mysql', 'query', 'SELECT id, name FROM users ORDER BY id LIMIT 1'], configDir)
  ...
  it('surfaces MySQL syntax errors', async () => {
    const {code, stderr} = await runCli(['mysql', 'query', 'SELCT 1'], configDir)

    expect(code).to.not.equal(0)
    expect(stderr).to.match(/You have an error in your SQL syntax/iu)
  })

  it('rejects multiple statements in one query', async () => {
    const {code, stderr} = await runCli(['mysql', 'query', 'SELECT 1; SELECT 2'], configDir)
