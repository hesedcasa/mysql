...
describe('mysql:auth:add', () => {
  ...
  const existingConfig = {
    ...
    profiles: {default: {database: 'testdb', host: 'localhost', password: 'secret', port: 3306, user: 'root'}},
  }
  ...
  beforeEach(async () => {
    ...
    mockInput = async ({message}: {message: string}) => {
      ...
      if (message.includes('MySQL host')) return 'localhost'
      if (message.includes('Port')) return '3306'
      if (message.includes('Username')) return 'root'
      if (message.includes('Password')) return 'secret'
      if (message.includes('Database')) return 'testdb'
      return ''
    }

    AuthAdd = await esmock('../../../../src/commands/mysql/auth/add.js', {
      '../../../../src/mysql/index.js': {testDirectConnection: mockTestDirectConnection},
