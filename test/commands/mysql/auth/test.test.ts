...
describe('mysql:auth:test', () => {
  ...
  const mockProfile = {database: 'testdb', host: 'localhost', password: 'secret', port: 3306, user: 'root'}
  ...
  beforeEach(async () => {
    ...
    AuthTest = await esmock('../../../../src/commands/mysql/auth/test.js', {
  ...
  it('successfully tests connection with valid config', async () => {
    ...
    expect(logOutput).to.include('Successfully connected to MySQL')
  })
  ...
  it('handles connection failure gracefully', async () => {
    ...
    expect(errorOutput).to.include('Failed to connect to MySQL')
  })
