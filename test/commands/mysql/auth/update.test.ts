...
describe('mysql:auth:update', () => {
  ...
  const existingProfile = {database: 'olddb', host: 'old-host', password: 'old-secret', port: 3306, user: 'old-user'}
  ...
  beforeEach(async () => {
    ...
    AuthUpdate = await esmock('../../../../src/commands/mysql/auth/update.js', {
  ...
  it('updates named profile with --profile flag', async () => {
    const configWithStaging = {
      ...
      profiles: {
        ...
        staging: {database: 'stagingdb', host: 'staging-host', password: 'stag-pass', port: 3306, user: 'stag-user'},
      },
    }
