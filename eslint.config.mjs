...
const config = [
  ...
  // eslint.config.mjs references typescript-eslint's API, which is only a
  // transitive dependency (via eslint-config-oclif) so it isn't listed
  ...
  {
    rules: {
      // mysql2 and the config layer legitimately use null (not undefined)
